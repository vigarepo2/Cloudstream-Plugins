...
export async function getExtensions(customUrls = []) {
  ...
  for (const item of rawList) {
    ...
    if (typeof item.type === 'string' && !item.tvTypes) {
      item.tvTypes = item.type.split(',').map(s => s.trim().toUpperCase());
    } else if (!item.tvTypes) {
      item.tvTypes = ["VOD"];
    }
