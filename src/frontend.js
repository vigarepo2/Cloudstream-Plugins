...
<body>
    ...
    <script>
      const App = {
        ...
        user: localStorage.getItem('cs_username'),
        data: null,
        ...
        renderLinksTable: () => {
            ...
            if(!App.data || App.data.length === 0) {
                tbody.innerHTML = '<div class="text-center py-10 text-gray-500 font-semibold bg-white rounded-3xl shadow-soft border border-gray-100">No access links created yet.</div>';
                return;
            }
                ...
                <div class="bg-white p-6 rounded-3xl shadow-soft mb-4 border border-gray-100 relative overflow-hidden \${!isActive ? 'opacity-75 grayscale-[0.5]' : ''}">
                    ...
                    <div class="grid md:grid-cols-2 gap-3 mb-5">
                        <div class="bg-gray-50 p-3 rounded-2xl flex justify-between items-center border border-gray-100">
                            <div><span class="block text-[10px] font-extrabold text-gray-400 uppercase tracking-wide mb-1">Standard Library</span><span class="text-xs font-mono text-gray-700 truncate block max-w-[200px] sm:max-w-[220px]">\${base}/\${l.link_id}/sfw/repo.json</span></div>
                        ...
                        <div class="bg-red-50/50 p-3 rounded-2xl flex justify-between items-center border border-red-100">
                            <div><span class="block text-[10px] font-extrabold text-red-400 uppercase tracking-wide mb-1">Adult Library (18+)</span><span class="text-xs font-mono text-red-800 truncate block max-w-[200px] sm:max-w-[220px]">\${base}/\${l.link_id}/nsfw/repo.json</span></div>
